...
Nothing flags **the task** when its value does not resolve to a calendar or
calendar-set note — for instance a link to a deleted note, a note that is
neither, an invalid calendar note, or a value that is not text at all (a number
or a checkbox). That task schedules exactly as if this property were blank.
