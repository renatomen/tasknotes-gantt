...
### Non-authored edge
A bar edge (start or due) whose date the user did not author — inferred from the other edge and an estimate, or placed at today when neither date exists. Signaled by a zigzag cut into that side of the bar; one signal covers what were previously separate "missing" and "inferred" treatments.
