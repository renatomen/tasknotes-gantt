...
### Placement boundary
The lint-gate rule that a ranked-defect junction file may not import the lifecycle-capture API of the debug-log module or touch the lifecycle global: instrumentation and diagnostics live in their own seam module, and junction files keep only the call hooks. It encodes the cohesion invariant directly — the first visible act of the 2026-08-21 regression was that import — so it is binary, not gameable by formatting, and fails at pre-commit (mechanized as U2 of plan `2026-08-23-001`; until it lands, the rule binds by review). A new allowed import is a boundary exception: a dated, structured allowance with its record (delta, why the seam cannot carry it, alternatives, the maintainer's approval), never a sentence. File size is not a gate (the 2026-07-30 ruling stands); it is published for review by the trend measurement.
