...
### Diff-sync
The refresh strategy in which the chart's store is seeded once and every later data change is applied as targeted store actions computed by an id-keyed diff against the last-applied state — chosen so the user's zoom, scroll, and selection survive every refresh. Its two blind spots are named and owned: a pure reorder is expressed as an explicit per-branch replay of move steps (a whole-branch chain keyed on an order fingerprint, deliberately simple rather than a minimal move set), because an id-keyed diff cannot reorder existing rows; and a wholesale resultset swing routes through a Bulk reseed.
