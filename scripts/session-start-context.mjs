...
export function heartbeatContract(root, source) {
  ...
  return [
    ...
    '  - VERDICT line present, receipt still missing, and neither a codex process carrying the',
    "    sentinel nor a wrapper bash whose arguments name this round's report file is alive (the",
    '    sentinel is minted inside the wrapper, so its process shows only its arguments): the',
    '    wrapper already exited nonzero (FINDINGS without --acknowledge, dirty tree, HEAD moved,',
    '    no sentinel): refused, no receipt is coming. Read its stderr and the report, fix, re-run.',
    '',
