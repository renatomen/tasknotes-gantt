...
function readSteps(root) {
  return [
    ...
    '     becomes an inline thread, and a thread count alone reports zero for it. Every review it',
    '     returns carries commit.oid — that field, not the review order, is what binds a review to',
    '     the head it reviewed, so read it before treating any review as an answer for step 3.',
  ];
}
...
function deliverySteps(root) {
  return [
    ...
    '     and no review body or top-level comment carrying a finding you have not addressed, or',
    '     deferred with a recorded maintainer acceptance. Recording a finding yourself is not',
    '     accepting it: the backlog entry is the record, the maintainer is the acceptance, and a',
    '     deferral you granted yourself leaves the finding outstanding. Never with an unresolved',
    '     final-gate thread. Zero threads also describes a',
    '     review that has not started, so require the hosted reviewer to have answered for the',
    '     exact headRefOid step 3 observed: a review BY THAT REVIEWER whose commit.oid equals it,',
    '     or their summary naming that commit — in full, or by an abbreviation you resolve and',
    '     confirm unique to that head, since a short prefix can name two heads.',
    '     Your own review carries commit.oid too and answers for nobody; an approving reaction',
    '     carries no SHA at all; and a review of an earlier head is not evidence for this one.',
...
function deliveryContract(root) {
  return [
    ...
    'and stop.',
    '',
    'A merge in step 6 ENDS this session rather than freeing it for the next task. Read that from',
    'the PR and not from the exit code: the merge happens before the branch deletion, so a failed',
    'deletion fails the command over a PR that is already merged. Re-read state and mergedAt after',
    'every merge attempt, and treat a merged PR as merged however the command exited.',
    'Once it is merged, delete every HEARTBEAT cron: --delete-branch has just removed the ref these',
    'commands poll, so a surviving heartbeat re-checks a closed PR against a branch that is gone.',
    'Then write the handover and stop.',
