...
- Open a PR for review. **Merge on green is the default**: passing CI (build, unit, e2e), both pre-push receipt layers recorded, and zero unresolved final-gate threads — the maintainer intervenes by exception, as product owner (charter E7 binding).
- **Right-size the review gate to the artifact.** Behaviour-bearing code gets the full adversarial loop: local layers, Codex, and zero unresolved inline findings (Codex files findings as inline review comments on the diff — a clean verdict summary is NOT clearance; count the threads). Docs-only PRs keep the mechanical local gate every push carries (both pre-push receipt layers) and settle final-gate threads **by finding class**, same as everything else: fix everyday-path factual defects however many rounds that takes; record or acknowledge the rest (charter binding, operational stopping rules). Prose is the cheapest artifact to correct, so fixing forward after merge is fine for residuals — and the definitive answer to a disputed factual claim is a reproduction or an experiment, never another review round.
- **Reports are snapshots.** A dated report (audit, retro, analysis) is immutable after merge; deferred or residual work items belong in `docs/backlogs/backlog.md` (parked work), and work being picked up immediately goes to GitHub Issues (active work) — never left inside the report. Maintaining a second living roadmap inside a report guarantees drift findings forever.
