...
- Assumes resolved children for a *currently-matched* parent (not any global edge) is a sufficient positive "relationships warmed" signal for early-stop. A matched set whose parents genuinely have no children cannot be distinguished from "not yet warmed" — handled by the attempt cap, accepted.
  - *As-built (PR #169, post-Codex-review): the early-stop signal is **mode-aware** — Show-all keys on a matched parent's resolved **children** (`childrenByPath`, the descendant-pull edge, exactly as worded above), Inherit keys on a matched task's resolved **parent** (`parentsByPath`, the nesting edge). The implementation plan briefly broadened this to "either edge regardless of mode," which Codex flagged: under partial warmup a parent edge can resolve before children, so the `OR` would false-stop Show-all and leave children absent. Mode-aware keying restores this assumption's intent.*
