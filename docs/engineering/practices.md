...
- **E11 — reviewer benchmarking is deferred.** Not implemented; bound honestly rather than claimed. No benchmark corpus exists yet, so reviewer selection is currently judgment, not measurement. **Revisit when a review layer's clean verdict is contradicted by a later round over unchanged text.** A deferral with no trigger defers forever — and that contradiction is the cheapest reliability measurement available, because detecting it needs no corpus, only two verdicts and a diff. Log each firing with both verdicts and the text they disagreed about; the log *is* the corpus this item says does not exist. First firing 2026-08-30 (PR #466): a clean peer verdict, then a later round raising two P1s in plan text that verdict had already read unchanged — and, earlier in the same series, a headline finding refuted against the code it described.
