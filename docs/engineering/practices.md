...
- **Mid-unit abort:** on context compaction or a state-class error (acting on stale state, contradicting a settled ruling), stop at the nearest green checkpoint, persist state, and end. The checkpoint is the last locally committed state: receipts are a push gate, not a stop gate, so an aborted session commits its work-in-progress without pushing, and the fresh session earns both receipt layers before any push. Never push through a degraded context to reach a merge.
