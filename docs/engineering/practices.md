...
**Test:** a session's chat transcript carrying a second PR merge; implementation work on a second unit or slice at any point — before or after its first merge (a multi-unit diff inside one PR is the landing cadence's own violation); any post-merge work product beyond closeout and maintainer conversation; a plan edited to record progress or mark units done; a direction change with no superseding plan naming the reason.
