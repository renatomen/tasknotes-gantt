...
Date: 2026-08-27. Trigger: maintainer question during the rank-1 style-block slice (PR #459) — "modularisation using size as a quality measure is a smell; are we really following Farley's approach?" Method: the book was read from the maintainer's vault (chapters 2, 3, 6, 9–14) and both the campaign method (`docs/reports/2026-08-15-001-maintainability-rediagnosis.md`) and the slice plan were audited against it, quote-first. This report is the durable record of that audit; the two campaign-method adjustments it produced are queued in `docs/backlogs/backlog.md` for a maintainer ruling at the next re-measure.
