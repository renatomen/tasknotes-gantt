...
Date: 2026-08-27. Trigger: maintainer question during the rank-1 style-block slice (PR #459) — "modularisation using size as a quality measure is a smell; are we really following Farley's approach?" Method: the campaign method (`docs/reports/2026-08-15-001-maintainability-rediagnosis.md`) and the slice plan were audited against Dave Farley, *Modern Software Engineering: Doing What Works to Build Better Software Faster* (Addison-Wesley, 2021), quote-first, working from the maintainer’s book notes on it (chapters 2, 3, 6, 9–14). Quotations below are brief and for commentary, per `docs/conventions/documentation.md` § Quoting external works. This report is the durable record of that audit; the two campaign-method adjustments it produced are queued in `docs/backlogs/backlog.md` for a maintainer ruling at the next re-measure.
