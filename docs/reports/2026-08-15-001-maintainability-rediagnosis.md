...
The two-file concentration is the headline: `GanttContainer.svelte` and `register.ts` together participate in a third of all commits ever made.
...
### `src/bases/GanttContainer.svelte` — 30 concerns (4,176 lines: 2,527 script / 278 markup / 1,369 style; 59 imports)
...
Structure facts: `initGantt` is 327 lines (1804–2130) holding 9 intercept call sites / 14 action registrations whose handlers close over and write seven outer mutable bindings (`syncing`, `ephemeralSort`, `pendingSingleClick`, `pointerButtonDown`, `lastCtrlMeta`, `hostGeneration`, `collapsedIds`) — the measured extraction trap the slice-2 plan carries. The two style concerns (~1,000 of 1,369 style lines) have zero script coupling.
...
- `test/unit/ganttSync.test.ts` (1,420 lines) — healthy companion; 16 top-level describes over distinct pure functions; would split cleanly along whatever lines its subject splits.
...
11. **RFC 7986 COLOR deviation** — tiny, decision-shaped: constrain `color` to CSS3 names or record the documented deviation in `docs/architecture/calendar-rfc-mapping.md`. Until decided, the mapping doc's lossless claim carries a known exception.
