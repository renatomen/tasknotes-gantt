...
| 6 | **Re-seat the zigzag on `wx-split`.** *(The swapped diagonal is SUPERSEDED — inverted dates render no bar and get a validation error badge instead; see docs/backlog.md "Schedule validation". Do not re-seat or resurrect it.)* The tooth-depth fitting exists *only* because host padding can widen the border box; under `wx-split` there is no host padding, so `fitToothDepth`, `measureSurvivingBorder`, `countTornSides` and the progress-wrapper re-pinning all disappear. **Design constraint:** a split host is transparent and hides its progress wrapper, and `.og-bar-body` merely *inherits* that transparency — so the re-seated template must paint the body from the published fill variable and re-render progress itself; ghost-run bars survive today only because their pieces carry their own fills. Narrow bars keep a fitting: the fixed depth returns only for bars wide enough to hold it — a one-day placeholder at coarse zoom is narrower than two teeth, and the fitting (or the per-surface ceiling) is what preserves a solid middle there, independently of the padding problem. | **~180 CSS + ~70 TS lines; ~50 of 71 `!important`s; the observer's `style` filter.** |
...
- Retire `datestatus-flagged` from the task-type registry. *(Correction: the audit ran on the U4 branch where the diagonal had displaced the colour CSS; on `main` the `.wx-bar.datestatus-flagged` orange/red rules are live and are the swapped state's only cue. Retirement therefore RIDES THE VALIDATION SLICE that replaces the cue — it is not independently shippable. The registry-bloat cost is real and waits with it.)*
- The initial-scroll reset at `GanttContainer.svelte:2118-2135` needs a truth-first cleanup: review evidence now shows the grid's real horizontal scroller is `.wx-scroll` (grid `Layout.svelte`), `.wx-grid` is a non-scrolling wrapper, `[data-id="grid"]` is never emitted, and `scroll-chart` moves the timeline, not the grid — so the block as written may be a six-way no-op, while an earlier review round held it live. Before any cleanup: reproduce the first-column-hidden symptom, fix against the real scroller if it reproduces, delete the block if it does not. Do not blindly prune or blindly keep.
...
- Move `scheduler.ts`, `calendarWatch.ts`, `checklistProgress.ts`, `types/bases-entry.ts` — and `services/BasesDataAdapter.ts`, which `BasesSource.ts` also imports across the same boundary — out of `bases/` into a neutral location; together they cause the six `datasource → bases` dependency inversions, and they block the cheap calendar-controller extraction the project wants to keep open.
