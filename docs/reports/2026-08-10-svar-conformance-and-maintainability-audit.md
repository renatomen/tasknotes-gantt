...
- Prune the scroll-reset at `GanttContainer.svelte:2118-2135` to its two live selectors (`.wx-grid`, `[data-id="grid"]`) or replace it with the public `api.exec("scroll-chart", …)` — three of its six selectors do not exist in SVAR 2.7.0, but the block itself is a live workaround for SVAR's occasional nonzero initial grid scroll, and deleting it wholesale would re-hide the first column on affected views.
