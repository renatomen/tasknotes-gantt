...
2. **(P0 as filed; severity votes P1/P1/P2 — treat as P1) Obsidian is globally mocked instead of bounded by a port.** `practices.md:87` (E9) states *"Third-party code is wrapped behind thin facades we own, never mocked directly"* with a governance test at `:89`; `jest.config.mjs:38` aliases a 449-line hand-written imitation for all 179 suites. **Corrected 2026-08-29:** this is *not* an unreconciled contradiction. `practices.md` § Repo divergences carries a maintainer ruling of 2026-08-14 — **E9 — DI-seamed mocks where a facade is impractical** — which sanctions exactly this arrangement for Obsidian (not for SVAR). The gap this audit found is therefore evidence bearing on that divergence's stated revisit trigger, *“if mock drift causes escaped defects”*, not a violation: the `BasesView` stub invents a wiring rule the published `QueryController` does not contain. No escaped defect is attributed to it. Whether that meets the trigger is a maintainer call. Aggravating: the `BasesView` stub (`obsidian.ts:149-155`) invents a wiring rule the published `QueryController` does not contain, and a test asserts against the invention.
...
**Step 4 — now plan `2026-08-29-001`'s required U4 (promoted 2026-08-30; that plan is authoritative for its status, this report is not):** `test/perf/generator/buildGanttData.ts` exports `assembleGanttData`, a **second producer of the render contract** — principle 4, second mechanism. Its own docstring states the justification: *"harness-local assembly, NOT a mirror of `register.ts`'s private `buildGanttData` — that method reaches into `app.vault` / `app.metadataCache` / `config.get`, which the in-memory harness lacks"*, so only perf-load-bearing fields are populated and the Obsidian-dependent ones are stubbed. **U3 removes that justification**: once the assembly sits behind the two ports, the harness supplies in-memory adapters and calls the real one, taking producers from 2 to 1. *(Verified here: the file, the export, and the docstring. The sequencer's field-level counts — 23 hardcoded literals, 12 fields absent — are its own and are not re-derived in this report; re-measure when the unit is picked up.)*

**Not recommended now:** the full port campaign (surviving gap 2). *(Corrected 2026-08-30: there is no charter violation to remedy — `practices.md` § Repo divergences sanctions the Obsidian arrangement, as lines above record. What gap 2 supplies is evidence bearing on that divergence's revisit trigger.)* The disagreement is sequencing. Steps 1–4 pay down its measured core. If the predicted movement does not materialise, the campaign argument gets stronger and should be re-raised **with that evidence**.

## Open items for maintainer arbitration

- **The E9 divergence's revisit trigger — a maintainer call.** *(Corrected 2026-08-29: the audit originally filed this as a live contradiction. It is not. `practices.md` § Repo divergences carries a maintainer ruling of 2026-08-14 — **E9 — DI-seamed mocks where a facade is impractical** — sanctioning this arrangement for Obsidian, with SVAR explicitly excluded. The audit reviewed E9's general statement at `:87` and missed the divergence at `:143` in the same file.)* What remains for the maintainer is narrower: the divergence says **revisit if mock drift causes escaped defects**, and one drift instance is now on record — the `BasesView` stub invents a wiring rule the published `QueryController` does not contain, and a test asserts against the invention. No escaped defect is attributed to it. Does that meet the trigger?
- **Severity of surviving gap 2** — filed P0, voted P1/P1/P2. *(Read with the correction above: what is for arbitration is the revisit-trigger question in the preceding bullet, not a contract violation.)*
- ~~Whether Step 4 becomes U4~~ — **settled 2026-08-30: it is a required U4** of plan `2026-08-29-001`, which owns the decision. Status questions about planned work belong to the plan; this report restating them is how these lines went stale in the first place.
...
- Dave Farley, *Modern Software Engineering: Doing What Works to Build Better Software Faster* (Addison-Wesley, 2021), consulted via the maintainer’s book notes; quotations are brief and for commentary, per `docs/conventions/documentation.md` § Quoting external works. Ch. 7 (Guided by Reality), Ch. 9 (The Importance of Testability; measurement points), Ch. 11 (Essential vs Accidental; Ports & Adapters), Ch. 12 (Fear of Over-Engineering; Isolate Third-Party Systems and Code), Ch. 14 (Measurement Points; Testing at the Edges).
