...
**Step 4 — recommended follow-up unit (not yet promoted):** `test/perf/generator/buildGanttData.ts` is a **rival producer of the render contract** — 23 hardcoded `GanttData` literals, 12 contract fields absent. Principle 4, second mechanism. U3's extraction makes it callable; retiring it takes producers of the render contract from 2 to 1.
