...
- **KTD4 — Option key `tngantt_highlightWeekends`, display name "Highlight weekends", Appearance group, default on (resolves the former open question).** Read as `get(key) !== false` so unset means on — the exact `showDateIndicators` pattern. Plumbing follows the standard chain: toggle in `viewOptions.ts` `appearanceOptions()` + pure `readHighlightWeekends()` reader → `register.ts` `buildGanttData` → `GanttData.highlightWeekends` → container derived.
