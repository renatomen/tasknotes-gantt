...
- **The peer wrapper would interpolate an unreviewed script's output into the reviewer prompt** — KTD5, U3 (P1, cross-model peer). Embedding the branch's own `maintainability-trend.mjs` output in the peer review's prompt lets a malicious or buggy branch print review instructions and steer the verdict before its own diff is reviewed. U3 delivers the trend block to the peer as data inside the diff payload (or an explicitly fenced, data-labeled attachment), never as prompt text, and runs the main-side script where feasible; the mitigation is stated in U3's PR.

- **The measurement window ends at the merge-base, so a stale branch's artifact omits newer main-side ranked changes** — KTD4, U3 (P1 as filed; informational-value class, cross-model peer). For a branch forked at M0 while main gained ranked-file change M1, the window ending at merge-base omits M1 although the PR merges onto it. Main-side changes are measured by their own PRs' artifacts, so the per-PR values stay honest for the PR's own delta — U3 records the window semantics in the artifact header (base SHA, window end) so a reviewer can see staleness, and its CLI test covers the forked-behind-main graph.

- **Three-dump-intersection parity can silently absorb the loss of a flaky-but-real event** — KTD8, U4 (P1, cross-model peer). An event present in two of three baseline dumps is excluded from the stable set, so the extraction could drop it entirely while parity stays green. KTD8 already requires between-baseline variance to be recorded in the PR as excluded; U4 additionally asserts each excluded event still exists in the post-extraction trace vocabulary (or names it in the intentionally-untraced enumeration AE9 requires), so exclusion-by-flake cannot become silent removal.

- **The lifecycle-global selectors are statically bypassable by computed names** — KTD1, R4/R7, U2 (P1, cross-model peer). `const key = '__tnGantt' + 'Lifecycle'; (globalThis as ...)[key]` defeats `Identifier`/`Literal`/static-`TemplateLiteral` selectors. Static analysis cannot close dynamic name construction in general; U2 records this as the boundary's known static limit, adds the split-string access as a documented negative case in the mutation-check appendix (observed passing, by design), and the review-layer invariant (any in-file diagnostics are a P1 by review) plus KTD7's structural census remain the guard for it.

- **The registry must seed the actual latest dated report, not only the baseline** — R8/KTD2, U3 (P2, hosted final-gate reviewer, citing the AGENTS.md rubric). The repository already holds dated trend reports after the 2026-08-15 baseline (latest at review time: `docs/reports/2026-08-17-002-rank3-u4-trend-report.md`, concern counts 29/14/14); seeding only the baseline would make the U3 artifact mislabel "latest report" and overcount "PRs since" until U4 lands. U3 seeds the `reports` array with every existing dated report (or at least the actual latest, with its anchor and counts) and states the seeding in its PR.

- **The R7 mutation proof cannot be pushed through the normal workflow as literally worded** — R7, U2 (P2, hosted final-gate reviewer). Each planted mutation must be observed failing pre-commit *and* in CI, but a commit pre-commit rejects cannot be pushed without bypassing hooks, which the git conventions forbid. U2 satisfies R7 with local per-mutation pre-commit failures (applied change printed, then reverted, per R7) plus a CI-side proof that does not require committing lint-invalid production files — a sanctioned CI fixture/harness exercising the same rules, or an equivalent the unit argues in its PR — never `--no-verify` pushes of known-red commits.
