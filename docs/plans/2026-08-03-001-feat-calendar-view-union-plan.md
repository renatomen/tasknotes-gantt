...
- R13. Enabling the recurring family suppresses that task's plain scheduled→due bar and span, matching calendar behavior; disabling it restores the plain bar while completed/skipped recorded-instance bars still render. A recurring task with no scheduled date produces zero instance bars and keeps its current row behavior.
...
- AE3. **Covers R3.** Given a vault whose calendar view shows ICS events and timeblocks by default, when the user creates a fresh Gantt view, then no calendar items render until they are toggled on.
