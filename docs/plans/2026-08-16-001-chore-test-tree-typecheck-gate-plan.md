...
- **Approach:**
  1. Configs extend root `tsconfig.json`, add `noEmit`, set per-framework `types` (KTD1/KTD5), disable `noUncheckedIndexedAccess` (KTD2). The jest config includes `test` and excludes only the e2e and vitest trees (KTD1 catch-all); the e2e config excludes `test/specs/_local-*` (R1).
