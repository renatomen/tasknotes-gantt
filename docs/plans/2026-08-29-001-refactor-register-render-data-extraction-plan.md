...
- **Test scenarios (all through the seam):**
  ...
  - Mapping-split pin (KTD3.3): with a status column selected through `getOrder`, the controller's effective mappings spied to name a status property the raw view config leaves unset, editor identity comes from the effective set and the progress and estimate write gates from the raw set.
  ...
  - Complete-field-set pin (KTD8b): a `Record<keyof GanttData, …>` fixture with a distinguishing non-default value per optional data field and a matcher per function- or Map-valued member (the derive closures and the drag-mode supplier are controller-built and cannot be literal); the assembled result's key set and values match it, so a dropped or added field fails.
