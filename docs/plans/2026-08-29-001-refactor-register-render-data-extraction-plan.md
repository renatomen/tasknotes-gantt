...
- **Test scenarios (all through the seam):**
  ...
  - Post-await census pin (KTD8a, mechanical): with the six controller reads held pending, spy every `get*` and `read*` method on the view's prototype, record each call against the resolution moment, resolve, and assert that every option read the pass made fired after resolution — the derived set is the test list, `arrowMode` the one named pre-await exception. Mutation check: hoist one option read (the date-indicators read, which U2's legend move touches) above the `Promise.all` on purpose, observe red, revert.
