...
- **Goal:** `buildGanttData`'s five behavioral decisions have Jest pins a later refactor cannot silently break, written against current behavior before any concern moves; the registration-factory harness becomes a shared helper every later characterization reuses. No production code changes.
...
- **Approach:**
  1. Extract the harness: the fake app (vault `getMarkdownFiles`/`getAbstractFileByPath`, metadata cache `getFileCache`/`getFirstLinkpathDest`, plus a swappable `plugins.getPlugin` and `metadataTypeManager.properties`), the fake config (`get` plus a recording `set`), the fake plugin that pockets the view factory, the plugin lifetime, and the real `GanttController` over the stub source exactly as `makeHarness` builds it today. `blockingBuilders.test.ts` switches to the import; a green run with no assertion change is the proof the extraction is behavior-preserving.
  2. Characterize: build the view through the pocketed factory, assign the harness's controller to the view's controller field through the same cast `blockingBuilders` uses (the one private write this plan makes, named here), `jest.spyOn` the seven async reads to deferred promises the test resolves, and call the assembly through the seam.
...
- **Test scenarios (all through the seam):**
  - Ordering pin (KTD3.1), value-based: with the name column's configured width set to a value other than the fallback and the table-width option unset, the assembled `gridWidth` equals that width only if the width write precedes the width read; reversing them on purpose turns it red.
  - Mapping-split pin (KTD3.3): with the controller's effective mappings spied to disagree with the raw view mappings, editor identity comes from the effective set and the progress and estimate write gates from the raw set.
  - Alias-first pin (KTD3.2): with the stored display-calendars object at its default and the weekend-highlight option off (the precondition under which the reconcile writes), the config `set` the reconcile performs is recorded before the first controller read starts.
  ...
  - Post-await liveness, three pins (KTD8a as amended): change the loading flag, the TaskNotes-present backing, and the user-field types while a controller promise is held pending; the completed result carries the new values. The TaskNotes flip also changes companion availability inside the field mappings, so the expected editor gates and write flags account for that side effect rather than failing for the wrong reason.
  ...
  - Complete-field-set pin (KTD8b): a `Record<keyof GanttData, …>` fixture with a distinguishing non-default value per optional field; the assembled result's key set and values match it, so a dropped or added field fails.
  - Derived-field consistency: `gridColumnsKey` equals the key derived from the result's own columns across two assemblies whose column order differs.
...
- **Cost-of-change probe (guardrail):** a new legend fact touches the input type, the projection, and its test — three places; today it touches the feeder block and the literal in `register.ts` — two places whose only test is a WDIO spec. The count rises by one and buys the Jest measurement point; the PR body says exactly that. Wiring ratio: plain-value fixtures, well under 1.0.
...
- **Cost-of-change probe (guardrail):** a new file-meta field touches the port type, the adapter, and the pass's test — three places; today it touches the inline adapter and the pass in `register.ts` — two places with no unit test. Wiring ratio: one fake port with a call log against the pass, well under 1.0.
