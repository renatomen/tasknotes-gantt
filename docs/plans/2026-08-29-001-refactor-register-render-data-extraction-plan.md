...
- **Scope guard — the crafted dataset is untouchable (maintainer knowledge, 2026-08-30).** This harness exists to reproduce the #161 render-loop storm, which the ordinary test tier could not trigger. Its data is deliberately calibrated — `presets.ts` `seed: 1`, the multi-parent fan-out (150/40/12 at 2/4/7 parents), `maxDepth: 6`, `depDensity: 0.1`, `cycleCount: 3`, `orphanCount: 6`, and the `dateMix` of 70% dated with undated / start-only / end-only tenths — calibrated to 31 / 980 / 3332 render instances under Show-all. **U4 changes the contract assembly only.** `graph.ts`, `presets.ts` and `toSources.ts` are out of scope: normalising that data would destroy the reproduction the harness was built for, and no tidiness argument outranks that.
