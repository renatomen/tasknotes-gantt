...
- **Test scenarios:**
  ...
  - Liveness, `externalEventsLoading`, **across the await window**: hold a controller promise pending, flip the backing field *while `getInstances()` is in flight*, resolve, and assert the completed result carries the new value. The between-assemblies form is insufficient — `onExternalBatchFlags` fires during snapshot collection, so an implementation that reads the bridge before `Promise.all` passes it while the loading indicator silently stays hidden.
  - Liveness, options port: change a port getter's backing value between two assemblies and assert the second result reflects it — with counters zeroed after construction so a cached read cannot satisfy it.
  - Liveness, `getInferredDragMode`, **after** assembly (Approach 1b): assemble, then mutate the backing option and call the *same result's* `getInferredDragMode()` without reassembling, asserting the new value. The between-assemblies form would pass a per-assembly snapshot, whose user-visible failure is that "don't ask again" prompts again on the next drag before the refresh round-trip.
  - `ResolveRenderType` timing (Approach 1a): with a controller promise held pending, change the user-field-type backing after the assembly starts, then resolve, and assert the produced cell renders use the **new** field types — pinning that the resolver is invoked after the awaits, not composed before them.
