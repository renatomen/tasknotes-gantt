...
- **Test scenarios:**
  ...
  - Liveness, options port, **between assemblies**: change a port getter's backing value between two assemblies and assert the second result reflects it — with counters zeroed after construction so a cached read cannot satisfy it.
  - Liveness, options port, **across the await window**: pick an option the current code reads *after* `Promise.all` (e.g. `showDateIndicators`), hold a controller promise pending, change its backing mid-await, resolve, and assert the completed result carries the new value. R4's no-read-across-`await` contract is a property of every post-await option read, not only of `externalEventsLoading`; the between-assemblies form alone would pass an implementation that snapshots the whole port up front and renders a slow pass with stale settings.
