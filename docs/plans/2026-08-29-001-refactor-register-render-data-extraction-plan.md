...
- **Verification:** `npm run lint`, `npm run typecheck`, full `npx jest` bare; **`npm run e2e:local -- --spec test/specs/gantt-legend.e2e.ts`, run unchanged as a regression gate.** U1 lands independently, and a same-typed adapter miswire — two colour facts or two booleans swapped — passes every Jest test in this unit because those call only the pure projection. U3's later legend run cannot protect a unit that merged before it. A red run is triaged under the reliability campaign's existing procedure, never "fixed" here.
...
- **Test scenarios:**
  ...
  - **Every adapter output exercised through a real context row**, not only through the fake port: a Show-all row resolved via the real `TFile`/metadata adapter renders correct `file.basename` and `file.extension` values. The fake-port tests cannot catch swapped adapter fields, and the selected e2e covers matched rows that bypass the adapter entirely.
