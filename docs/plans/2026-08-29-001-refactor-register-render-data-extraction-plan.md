...
- **Test scenarios (all through the seam):**
  ...
  - Mapping-split pin (KTD3.3), both halves observable: select a status column **and** the progress and estimate columns through `getOrder`; spy the controller's effective mappings to name a status property the raw view config leaves unset, and choose raw and effective progress/estimate mappings whose writability outcomes are opposite. Assert that the status column's editor identity comes from the effective set, and that the progress and estimate editors are present or absent exactly as the **raw** set dictates (the cell-editability resolver consults the write gates only for those two columns, so a status-only fixture would stay green if the gates were re-derived from the effective set).
