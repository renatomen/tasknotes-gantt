...
- KTD8. **Contracts stated as rules with mechanical censuses, scoped to what actually moves.** *(Amended 2026-09-03.)* The complete-field-set pin (b) stands: at U2's projection seam for its input and output, and at U1's assembly characterization for the full `GanttData` key set — written as a `Record<keyof GanttData, …>` fixture (the compile-time shape the repository settled for derived member lists), never a hand-typed name array, so a field added to the interface fails typecheck until the pin covers it — with a distinguishing non-default value per optional field, so a dropped field fails rather than passes. The post-await read contract (a) is stated as a **contract over read occurrences**, never as a mechanism — six peer rounds showed that every mechanism named in a plan can be satisfied while the contract is broken, so the plan names none (R5b). Contract (R4): every read the pass performs keeps its current side of the fan-out. A read occurrence, not a key or a member, is the unit: the same source may be read on both sides (the two calendar-selection keys are read before the await by the alias reconcile and again after it by the shading builder and the highlight field), and each occurrence is classified where the code performs it. The characterization enumerates the occurrences from the code — Bases config keys, host members, the view's own data, process-level inputs such as the debug locale global — and proves each one on its side by changing the source while the controller reads are held pending and asserting which value each occurrence observed. The mutation that must turn it red: moving any one occurrence across the fan-out. The loading flag, the TaskNotes-present check, and the user-field types are occurrences whose backing is a live host fact; they are named because their side effects (companion availability, empty field types) shape the expected values. The single-snapshot rule (c) keeps its two named instances (`userFieldTypes` serving both cell rendering and editor resolution; `arrowMode` read once and handed to the links read), and the fifth decision — the drag-mode supplier crossing the render contract as a function, never resolved during assembly — gets its own pin.
...
- **Approach:**
  ...
  2. Characterize: build the view through the pocketed factory; assign the harness's controller to the view's private `ganttController` field through the same cast `blockingBuilders` uses **and** pass that same instance as the assembly's argument (the effective-mappings read goes through the field, the reads through the argument); put the controller's async reads under the test's control so they can be held pending and resolved on demand; call the assembly through the seam. The plan's private writes are two and named: `ganttController` here, and `externalEventsLoading` for the loading-flag occurrence (only the mount's observer sets it in production). How the reads are held, recorded, or spied is the characterization's to discover (R5b); the plan states what each pin proves and what must turn it red.
...
- **Test scenarios (all through the seam; each states the contract and the mutation that must turn it red — mechanisms are the characterization's, per R5b):**
  - Width ordering (KTD3.1): the assembled grid width is derived from the width written during this pass, not from a stale field; on a freshly built view's first assembly, with the table-width option unset and the name column's configured width away from the fallback, the result carries that width. Red: perform the width read before the write.
  - Mapping split (KTD3.3): editor identity comes from the effective mappings and the progress and estimate write gates from the raw view config; the identity probe is an ungated column (the start date — a status column is also gated by the controller's status-writable flag, false on the harness controller, so it cannot tell a correct identity source from a wrong one), and the progress and estimate columns carry raw and effective mappings with opposite writability. Red: derive either half from the other set.
  - Alias first (KTD3.2): the calendar-selection reconcile runs, and any config write it performs lands, before the first controller read starts; the fixture is the one under which it writes (the display-calendars option stored as an object with an explicit `default: true`, the weekend-highlight option `false`); with a fake config nothing refreshes, so expect one write per assembly. Red: move the reconcile after the fan-out.
  - Fan-out: all seven controller reads (six methods, one called twice) are started before any resolves, and the assembly stays pending while one is outstanding. Red: make the reads sequential.
  - Post-await read contract (KTD8a): every read occurrence keeps its current side of the fan-out — enumerated from the code, including the debug locale global and the view's own data, and proven by changing every source while the reads are held pending. Red: move any one occurrence across the fan-out (the date-indicators read, the row-visibility reader, the locale resolution, and the arrow-mode read are the four to try).
  - Single snapshot (KTD8c): the arrow mode read once per pass is the value the links read receives; the user-field types resolved once serve both cell rendering and editor resolution. Red: read either twice with different backings.
  - Drag-mode supplier: the render contract carries the supplier, not its value — mutate the backing option after assembly and the same result's supplier answers the new value. Red: resolve it during assembly.
  - Complete field set (KTD8b): the result's key set matches `GanttData` through a `Record<keyof GanttData, …>` fixture with a distinguishing non-default value per optional data field, and every function-valued member is pinned by calling it through the result (the refresh-generation member returns the controller's next generation when invoked detached; the derive closures answer what the controller answers). Red: drop a field; bind a method reference bare.
  - Derived field: the grid-columns key equals the key derived from the result's own columns, across two assemblies whose column order differs. Red: return a constant key.
  - Every mutation above is run, observed red, reverted, and the applied change printed as evidence.
