...
- KTD8. **Two contracts stated as rules with mechanical censuses, not as per-member scenario lists.** Three peer-review rounds enumerated members crossing the same two boundaries one at a time (`externalEventsLoading`, then `getInferredDragMode` and `ResolveRenderType`, then `isTaskNotesPresent`, `arrowMode` and `dateMappingNotice`). A hand-maintained list of members is incomplete by construction; the classes are what must be pinned.
  ...
  - **(c) One snapshot, every consumer — the list derived, never named.** Any value the pass reads **once** and hands to more than one consumer owes a pin that *every* consumer received that same snapshot; the consumer list comes from the characterization's read census, not from this sentence. Two instances the characterization already owes. **`userFieldTypes`:** the post-await snapshot feeds both cell rendering (`register.ts:1500-1504`) and inline-editor resolution (`:1556-1565`), so the `ResolveRenderType` provider must serve both from one snapshot and the pin asserts the `cellRenders` result **and** the custom-field `cellEditors` descriptor — otherwise a stale or empty lookup renders an enabled custom field normally while leaving it read-only or giving it the wrong editor. **`arrowMode`:** read **once** before `Promise.all` and must reach both `controller.getLinks(arrowMode)` and the returned `arrowMode` field — the same value, not two reads. Pin one getter read for the pass and assert the exact argument `getLinks` received equals the reported field; two reads would let a mid-pass mode change generate `primary` links while the result claims `all`, dropping secondary-instance edges.
...
- **Test scenarios:**
  ...
  - **Derived-field consistency pin.** A field that is a *derivation of another returned field* is asserted **against that field**, never merely for presence — KTD8b checks keys and distinguishing values and would pass a constant. Today's instance: `result.gridColumnsKey === gridColumnsKey(result.gridColumns)`, pinned across **two** assemblies whose column order or header differs, asserting the key actually changes. `ganttSyncOrchestrator.ts:349-360` reseeds SVAR only on a key change, so a constant or stale key leaves a user's reordered or renamed columns showing the old layout. Mutation-check with a constant key.
...
- **Scope guard — the calibrated dataset stays put.** Its history, from `docs/solutions/`, because the obvious assumption is wrong: this harness does **not** reproduce the #161 render-loop storm and never did. It was built (PR #162) around a *static-render-scale* hypothesis, and `developer-experience/match-harness-execution-model-to-bug-trigger.md` records that its execution model is “open the view once, measure the render” — one trigger, and not the storm's. The storm was **real-data-only** (`integration-issues/gantt-bases-getvalue-renotify-storm.md`: it reproduced *only* on the production vault, never on synthetic vaults matching config shape and scale) and was root-caused with console flags in the live vault. The harness's *green* run was the value — it **disproved** the static-scale hypothesis. What makes the data load-bearing is therefore calibration, not reproduction: `presets.ts` is the single source of truth for three consumers (isolated gate, full-stack spec, diagnosis CLI) so the calibration “never drifts between consumers” — `presets.ts` `seed: 1`, the multi-parent fan-out (150/40/12 at 2/4/7 parents), `maxDepth: 6`, `depDensity: 0.1`, `cycleCount: 3`, `orphanCount: 6`, and the `dateMix` of 70% dated with undated / start-only / end-only tenths — calibrated to 31 / 980 / 3332 render instances under Show-all. **U4 changes the contract assembly only.** `graph.ts`, `presets.ts` and `toSources.ts` are out of scope: changing them breaks comparability across those three consumers and against the recorded reference points, and no tidiness argument outranks that. (The gate itself runs weekly and on demand — `.github/workflows/perf.yml` — not per PR.)
- **Scope guard — keep the measurement isolated.** `propertyValues` and `gridColumns` are stubbed today with a stated reason: they do not affect the row-count and timing metrics the gate measures. Calling the production assembler must not smuggle that work into the timed path. Supply deliberately minimal port adapters so there is **one producer and an unchanged measurement**; if a field cannot be kept out of the timing, report the before/after numbers and take the trade to the maintainer rather than absorbing it. (Note the duplication is already narrower than the audit implied: the harness wires the real `GanttController` and imports the production `buildGridColumns`, so only the final contract assembly is local.)
- **Scope guard — the fixed locale is part of the measurement.** The generator pins `dateLocale: 'en-US'` (`test/perf/generator/buildGanttData.ts:108`); production resolves it at runtime (`resolveDateLocale()`, `src/bases/register.ts:1509`), and weekend derivation follows from it. Calling the production assembler must supply the locale **through the port**, never resolve it from the runner: on an `ar-SA` runner the weekend moves from `[6,7]` to `[5,6]`, changing rendered classes — and possibly timing — for the identical calibrated graph, which silently breaks both this unit's unchanged-measurement claim and cross-run comparability. Treat any other host value the assembler resolves at runtime the same way; the port list is derived from what the production assembler reads, not from this sentence.
