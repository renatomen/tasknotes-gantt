...
- **Recommended follow-up unit, not promoted here (maintainer's call):** `test/perf/generator/buildGanttData.ts` is a **rival producer of the render contract** — 23 hardcoded `GanttData` literals with 12 contract fields absent entirely, a second mechanism for the job U3's module will own (principle 4). U3's extraction makes the real assembler callable through in-memory adapters for the two ports, taking producers of the render contract from 2 to 1. Recorded by the 2026-08-29 audit's sequencer; promote to a unit or leave to a later plan.
