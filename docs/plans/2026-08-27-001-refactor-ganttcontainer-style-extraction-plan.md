...
- R2. The extraction is behavior-preserving with a mechanical proof, staged in two steps. The **relocation** is proven by byte-identity: `dist/styles.css` and `dist/main.js` (sourcemap content excluded) byte-identical to a named baseline receipt captured from the pre-change tree. The **final comment-only commit** (R3's second step) is proven by two receipts: source CSS with comments stripped byte-identical before/after, and compiled `dist/styles.css` (scope-hash-normalized) differing from the baseline only in comment text.
- R3. Zero CSS rule edits, in two sanctioned steps: first an exact-bytes relocation — original inner bytes, splice whitespace included, no added header comment, because the Svelte scope-hash class derives from the style content — then, once the byte-identity receipts are recorded, one comment-only commit that makes the block's pre-existing volatile-reference comments self-contained (final-gate requirement: the landed owned file carries no volatile refs and no guard bypass), verified by R2's comment-stripped receipts. Rule bytes are never edited. Pre-existing style defects discovered during the move (e.g., the duplicated touch-rule group at ~2784–2797 / ~2960–2969) are recorded in `docs/backlogs/backlog.md`, never silently repaired.
