...
- **Test scenarios:**
  ...
  - Occurrence pieces on a swapped bar keep their per-state cues — enumerate and assert every occurrence state the occupancy renderer distinguishes (as defined in its state map at implementation time), not an ad-hoc pair; background-borne cues win their piece, overlay-borne cues layer above the diagonal (KTD4).
