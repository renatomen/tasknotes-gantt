...
- **Approach:**
  ...
  2. In `BarContent`, skip the piece branches (`ghostPieces` / `occupancyView`) only when the swapped visual is active — the bar carries the swapped token (indicators on) — so the bar renders continuous for the diagonal; with indicators off, split rendering stays untouched (R9).
