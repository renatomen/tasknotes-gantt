...
- **Test scenarios:**
  ...
  - Continuous strip-treated bar with a left zigzag: the host mask clips the strip accent into the teeth; both stay legible.
  - Split strip-treated bar with a left-edge token: the strip accent does not cover the outer piece's teeth (offset or mask the strip per KTD3).
  ...
  - Suppressed recurring envelope at coarse zoom (spine-only rendering): the spine's outer end carries the mask per KTD3.
...
- **Dependencies:** U3.
- **Files:** `src/bases/GanttContainer.svelte`, `src/bases/BarContent.svelte` (swapped bars skip the piece branches), `src/bases/ganttSync.ts`, `src/bases/barTreatment.ts` (publish the dedicated fill variable from every fill rule), `test/unit/barTreatment.test.ts`, `test/specs/gantt-date-handling.e2e.ts`.
- **Approach:**
  ...
  2. In `BarContent`, skip the piece branches (`ghostPieces` / `occupancyView`) for swapped tasks so the bar renders in its continuous form and the diagonal covers the whole bar (KTD4).
...
- **Test scenarios:**
  ...
  - Swapped task under Split rendering renders as a continuous bar (no pieces), so the diagonal covers the whole bar area per KTD4.
...
- **Test scenarios:**
  ...
  - Rows hidden by display filters (show-undated, show-partial, hide-top-level, session sources) do not count toward presence; ancestors retained only as tree scaffolding by the filter do not count either.
