...
- **Zigzag as a CSS mask carved into the bar.** The teeth are cut out of the bar element itself rather than drawn over it, so the signal composes with any fill and adds no DOM. (session-settled: user-directed — chosen over overlay or border approaches: the user supplied the conic-gradient mask technique; see Sources.) Governs R2.
- **Swapped dates render as a full-bar diagonal half-fill.** (session-settled: user-directed — chosen over a small half-filled square icon chip: no contention for the single chip slot, legible at any bar width, and icon chips render above it.) Governs R5.
...
- **The signal survives Split rendering.** (session-settled: user-approved — chosen over continuous-bars-only: the missing-edge signal is semantics, not decoration.) Governs R4.
...
- R4. Under Split rendering, the outermost visible piece carries the zigzag on its outer edge; at zoom levels where split tiling falls back to a continuous bar, the zigzag renders on the continuous bar's edge.

**Swapped indicator**

- R5. A bar with swapped dates renders a borderless diagonal half-fill across the whole bar area — one triangle in the bar's fill color, the other transparent — with any icon chip rendered above it, and receives no zigzag.
...
- CSS zigzag-edge technique: [CSS Borders Using Masks](https://css-tricks.com/css-borders-using-masks/) (conic-gradient mask, repeat-y per edge). Chosen-treatment mock: [docs/mocks/missing-date-zigzag-options.html](docs/mocks/missing-date-zigzag-options.html).
...
- Institutional learnings that constrain the mechanism: [docs/solutions/integration-issues/svar-gantt-injected-css-scoped-specificity.md](docs/solutions/integration-issues/svar-gantt-injected-css-scoped-specificity.md), [docs/solutions/integration-issues/svar-gantt-datauri-currentcolor-glyph-invisible.md](docs/solutions/integration-issues/svar-gantt-datauri-currentcolor-glyph-invisible.md), [docs/solutions/conventions/svar-gantt-bar-geometry-and-fill-conventions.md](docs/solutions/conventions/svar-gantt-bar-geometry-and-fill-conventions.md), [docs/solutions/integration-issues/svar-shared-classname-selector-leak.md](docs/solutions/integration-issues/svar-shared-classname-selector-leak.md), [docs/solutions/architecture-patterns/context-aware-legends-project-orthogonal-semantics.md](docs/solutions/architecture-patterns/context-aware-legends-project-orthogonal-semantics.md).
...
- KTD1. **Per-state type tokens ride the existing task.type channel.** Four new class tokens (zigzag-left, zigzag-right, zigzag-both, swapped) replace the single `datestatus-flagged` flag, resolved from `dateStatus` in `ganttSync` and registered in the existing `buildTreatmentTaskTypes`/`buildInstanceCueTaskTypes` cross-products. A bar carries one type string; four tokens is the minimum distinct set, and the `taskTypes` array must stay reference-stable at runtime (a new reference re-initializes SVAR's store). Governs the mechanism for R1 and R5.
- KTD2. **Zigzag is an alpha-only CSS mask in the view stylesheet.** Conic-gradient masks per the settled technique (see the Product Key Decision governing R2), painted with `mask`/`-webkit-mask`, never a data-URI SVG with `currentColor` (resolves in the wrong document context — see Sources learnings). Rules anchor at `.og-bases-gantt` with `!important` because SVAR's Svelte-hashed styles out-specify plain injected rules. The masked side drops its corner radius. Tooth period is 8px per R3, carried as a CSS custom property (TS constants cannot interpolate into a Svelte style block) and asserted in tests.
- KTD3. **Split bars mask the outermost piece, not the host.** A `wx-split` host is `background-color: transparent`, so the mask targets the outermost `.og-ghost-run` pieces — via explicit hook classes stamped from the piece-rendering loop in `BarContent.svelte` (first index and last index), because the container's last DOM child is the bar-content element, so `:last-child` would never match a piece. A single-piece bar carries both hooks and receives both edge masks as combined mask layers. The mask targets the outermost piece regardless of whether it is a blocked (dimmed) piece; U2 verifies legibility on a dimmed outer piece and escalates if it fails. The coarse-zoom fallback needs no code: when tiling is off, `ghostPieces` is null and the plain continuous-bar branch (and its mask) applies. The old `.wx-split:not(.datestatus-flagged)` border carve-out is deleted in U4 together with the color CSS. Covers R4.
- KTD4. **Swapped half-fill is a diagonal gradient fed by a published fill variable.** The bar's fill color is not readable from CSS `background-color`, so the diagonal renders as a `linear-gradient(135deg, <fill> 50%, transparent 50%)` background whose color arrives via the inherited custom property the fill rules already publish for composite pieces (`--og-ghost-fill` chain); if a fill path doesn't publish it, publish a dedicated variable rather than masking the bar element (a bar-level alpha mask would also hide labels and chips). The swapped rule must also force `background-color: transparent !important` above the generated fill rules — a gradient is a background-image layered over the solid fill, so without defeating it the "transparent" triangle shows the fill itself (precedent: the `.wx-bar.wx-split` transparent override). Icon chips need no change — they render in `.wx-content`, above the bar background. Inherits the session-settled label of the Product Key Decision governing R5.
...
- The mask on the bar element leaves progress fill and bar text legible (the mask cuts only edge pixels); U2's e2e scenarios verify this and the fallback is masking an inner wrapper instead.
- The `--og-ghost-fill` variable chain is publishable/reusable for the diagonal gradient (KTD4); U4 verifies and may introduce a dedicated variable.
...
```mermaid
flowchart TB
  T["dateStatus token on task.type"] --> C{"rendering mode"}
  C -->|continuous bar| B[".wx-bar.<zz-token>\nmask on the bar element"]
  C -->|split with pieces| P[".og-ghost-run:first-child / :last-child\nmask on outermost piece"]
  C -->|coarse zoom fallback| B
  T -->|swapped| S[".wx-bar.<swapped-token>\ndiagonal gradient via published fill var"]
```
...
- **Dependencies:** None.
- **Files:** `src/bases/visualSemantics.ts`, `src/bases/ganttSync.ts`, `test/unit/ganttSync.test.ts`.
- **Approach:**
  1. Add four class tokens to `GANTT_VISUAL_CLASS_TOKENS` and a `dateStatus → token` resolver.
  2. In the type-string composition, append the per-state token alongside the existing flag (which still gates on `showDateIndicators`, per R9).
  3. Extend `buildTreatmentTaskTypes` and `buildInstanceCueTaskTypes` so every new combination is pre-registered; keep the `taskTypes` reference stable.
...
- **Test scenarios:**
  ...
  - Indicators toggle off → no date-status tokens and no flag in the type string.
  - Registration sets contain the new token combinations; existing combinations unchanged.
  - `taskTypes` reference stability across data updates is preserved.
- **Verification:** Full jest green; existing e2e untouched and green (old class still present).

### U2. Zigzag mask rendering (continuous bars and split pieces)

- **Goal:** The three zigzag tokens render the torn edge — on the bar element for continuous bars, on the outermost ghost piece under Split rendering. Old colors remain active this PR.
- **Requirements:** R1, R2, R3, R4; KTD2, KTD3.
- **Dependencies:** U1.
- **Files:** `src/bases/GanttContainer.svelte`, `src/bases/BarContent.svelte` (outer-piece hook classes), `test/specs/gantt-date-handling.e2e.ts` (additive assertions).
- **Approach:**
  1. Add mask CSS per KTD2 (left / right / both variants, 8px constant, masked side radius 0), scoped `.og-bases-gantt` + `!important`, selectors container-scoped to bar context (class-name leak learning).
  2. Stamp the outer-piece hook classes and add the split-piece selectors per KTD3, including the combined single-piece rule.
- **Execution note:** Start with a failing e2e assertion reading the computed `mask-image` on a zigzag-class bar.
- **Test scenarios:**
  ...
  - Hover and selection highlight remain a clean rectangle on a masked continuous bar; if the mask clips them, move the highlight to a sibling overlay outside the masked box.
  - A strip-treated zigzag bar keeps its strip accent legible where the teeth cut the same edge.
...
- **Verification:** Full jest; `npm run e2e:local` gantt-date-handling spec green including new assertions.
...
- **Approach:**
  ...
  2. Keep the `.wx-bar.datestatus-flagged` color CSS and the `.wx-split:not(.datestatus-flagged)` carve-out live — they now apply only to swapped bars; deletion moves to U4 so no state is ever unsignalled.
...
- **Dependencies:** U3.
- **Files:** `src/bases/GanttContainer.svelte`, `src/bases/ganttSync.ts`, `src/bases/barTreatment.ts` (publish the fill variable if a path lacks it), `test/unit/barTreatment.test.ts`, `test/specs/gantt-date-handling.e2e.ts`.
- **Approach:**
  ...
  2. Remove the old-flag assignment for `swapped` in `ganttSync`, then delete the `.wx-bar.datestatus-flagged` color CSS block and the `.wx-split:not(.datestatus-flagged)` carve-out — this PR retires their last consumer.
...
- **Goal:** The legend replaces the two color entries with the zigzag entry (both-edge swatch) and the swapped entry (diagonal-split swatch), context-gated as before.
- **Requirements:** R6, R7; KTD5.
- **Dependencies:** U2, U3, U4 (the treatments the swatches project; U3 transitively via the PR chain).
- **Files:** `src/bases/visualSemantics.ts`, `src/bases/legendCatalog.ts`, `src/bases/GanttLegend.svelte`, `src/bases/register.ts`, `src/bases/types/gantt-view-data.ts`, `test/unit/legendCatalog.test.ts`, `test/specs/gantt-legend.e2e.ts`.
- **Approach:**
  ...
  3. Add the R7 gating as new mechanism: extend `GanttLegendContext` with the indicator toggle plus per-state presence flags (any non-authored-edge task; any swapped task) computed at the `register.ts` context-assembly seam (precedent: `hasRecordedRecurringOccurrences`), and filter the two entries in `buildLegendCatalog`.
