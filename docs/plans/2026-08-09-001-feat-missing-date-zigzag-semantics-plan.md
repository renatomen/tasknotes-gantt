...
> **CLOSED 2026-08-10 — executed with deviations. Do not execute any part of this plan.**
> U1–U3, U5 and the docs half of U6 shipped (#395/#397/#398/#402/#406). Every
> swapped-date contract in this body — the diagonal in the Objective, the
> state-mapping row, R5/R6/R7/R9, AE4/AE5/AE8, the Scope Boundary forbidding
> validation, KTD4, U4 and the DoD entry counts — is superseded: inverted dates
> render **no bar** and receive a **validation error badge** (first slice of the
> schedule-validation feature; see docs/backlog.md). The Deviations section at
> the end records the full close-out.

## Goal Capsule

- **Objective:** Replace the color-based date-status treatment (orange fill + red border) with shape-based signals: a zigzag "torn" edge on the side of each non-authored date, a borderless diagonal half-fill for swapped dates *[superseded - see the CLOSED banner]*, and matching legend entries.
...
| swapped (start > due) | borderless diagonal half-fill of the bar; icon chips render above; no zigzag *[superseded - see the CLOSED banner]* |
...
- R6. *[superseded - see the CLOSED banner]* The legend's two date-status color entries are replaced by two new entries: a zigzag entry whose swatch shows teeth on both edges, meaning an edge that was not authored, and a swapped entry showing a diagonal-split swatch with its meaning.
- R7. *[superseded - see the CLOSED banner]* The new entries are shown only when the view contains matching tasks and the date-status indicator option is on. The zigzag entry matches any task with a non-authored edge (inferred-start, inferred-end, placeholder); the swapped entry matches swapped tasks; presence is computed from the view's rendered instances. This gating is new mechanism — the legend does not gate entry visibility today.
...
- R9. *[superseded - see the CLOSED banner]* The existing "Show date-status indicators on bars" view option gates the zigzag edges and the swapped half-fill together; no new setting is introduced.
...
- AE5. *[superseded - see the CLOSED banner]* **Covers R9, R7.** Given the date-status indicator option is off, then no bar shows a zigzag or diagonal half-fill and the two new legend entries are absent.
...
- No warning, validation, or auto-fix for swapped dates *[superseded - see the CLOSED banner]* - reversed: swapped dates became the first validation error.
...
- The legend shows exactly the two new date-status entries, context-gated. *[superseded - see the CLOSED banner]*
