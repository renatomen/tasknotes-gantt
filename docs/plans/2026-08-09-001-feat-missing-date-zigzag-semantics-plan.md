...
- **Swapped dates render as a full-bar diagonal half-fill.** (session-settled: user-directed — chosen over a small half-filled square icon chip: no contention for the single chip slot, legible at any bar width, and icon chips render above it.) Governs R5. Conflict note from planning review: one evidence-based exception is recorded on R5 — split pieces whose background carries an occurrence-state cue keep the cue instead of their diagonal slice.
