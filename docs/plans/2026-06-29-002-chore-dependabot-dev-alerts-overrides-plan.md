...
That backlog is **12 open alerts** (matching the count noted in #181), all **development-scope, transitive** dependencies (production audit `npm audit --omit=dev` remains **0 vulnerabilities**). #181 assumed these were gated behind `--force` **breaking major bumps** of the e2e framework and deferred them. This plan tests a surgical alternative #181 did not attempt: a **clean lockfile regeneration** (which upgrades several transitive deps to already-published patched lines) plus **scoped npm `overrides`** that pin patched versions where the natural resolution does not reach them — then verifies the full harness still passes. Each override that breaks the toolchain falls back to an explicit, documented hold rather than a force-fix.

> **Scope correction (during execution):** an initial query underreported the open set as 3 alerts due to API pagination. The true open set is **12 alerts across 6 packages**: `js-yaml` (#3, #4, #88), `@babel/core` (#87), `serialize-javascript` (#36, #84), `glob` (#5), `minimatch` (#30, #31, #32), and `picomatch` (#57, #58). All are resolved by npm `overrides`, built minimally on top of `main`'s lockfile to avoid churn (an early full-regen attempt was discarded because it incidentally bumped `eslint` 9.36→9.39 and dropped the transitively-resolved `@eslint/js`, which would have broken lint on CI). The override set: `@babel/core`/`serialize-javascript` (top-level); `js-yaml` scoped per-consumer (4.2.0 for `mocha`/`@eslint/eslintrc`, 3.14.2 for the EOL `@istanbuljs/load-nyc-config` 3.x instance); and version-selective `glob@10`/`minimatch@3,5,9`/`picomatch@2,4` to bump only the vulnerable lines within their majors.
