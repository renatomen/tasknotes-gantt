...
- KTD2. **The estimate-only cascade journey runs over the all-blocked calendar, not the working-week seam** (session-settled: user-directed — chosen over dropping it as planner-covered: the echo-guard and entry-signature interplay is observable only in real Obsidian). The `Blocked Parent` / `Blocked Child` fixture pair already exists for exactly this case and is currently rendered by the seam base but never dragged, so the journey needs no new fixture authoring. Governs R2.
