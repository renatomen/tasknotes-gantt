...
- **Slice-2 planning (next session)** — Depends on nothing here; runs via ce-plan as a fresh plan. The 2026-08-12 plan on `docs/plan-wire-svar-interceptors` (`docs/plans/2026-08-12-001-refactor-wire-svar-interceptors-plan.md`, which exists only on that branch — deliberately not linked, since a relative link would 404 on `main`) is superseded reference-only and receives its tombstone per the charter. The fresh plan carries forward verbatim its measured seven-bindings trap: `initGantt`'s intercept registrations close over at least nine outer mutable dependencies — `syncing`, `ephemeralSort`, `pendingSingleClick`, `pointerButtonDown`, `lastCtrlMeta`, `hostGeneration`, `collapsedIds`, plus the mutable `api` binding and `suppressSelectActivation` (the re-measured read/write split is in the Phase 0 report) — writing some and reading others written by the outer capture listeners, so a naive deps-object extraction compiles and smoke-tests green while silently breaking echo suppression or selection-activation suppression. The slice-2 plan re-enumerates this set at planning time rather than trusting a fixed name list. Precedent slices: #416, #418.
