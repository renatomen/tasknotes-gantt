...
- KTD2. **Accept a binary only when the repository declares it binary.** Under the KTD1 view, `diff --numstat -z --no-renames` marks each content-binary path with `-` counts, on both sides of a rename. Each such path must carry the `binary` attribute in the committed attributes of the reviewed commit (`check-attr --source=<reviewed sha>`, global file off), and must not carry it when the same query reads the empty tree, which exposes a declaration from `.git/info/attributes` or the system file. Both sides of the pair must be binary on their own, because git calls a pair binary when either side is, so text replacing an image (or the reverse) would pass unread. Any other refuses with exit 14. The committed `.gitattributes` already declares the image types, so no new list is kept. A git failure in either step refuses with exit 10. NUL-separated output goes to a temp file under the existing EXIT trap and is read with NUL-delimited `read`; it is never held in a command substitution or read through a pipeline, because bash drops NUL bytes and a pipeline reports only its last status.
