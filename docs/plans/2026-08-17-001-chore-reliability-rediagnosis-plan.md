...
- KTD2. **Reporter output lands under `.wdio-results/`.** The `.wdio-*` prefix is already covered by `.gitignore`, ESLint ignores, and the `ci.yml` e2e artifact upload glob — zero configuration changes, and no untracked files in developer working trees (a bare `test-results/` path is not gitignored).
