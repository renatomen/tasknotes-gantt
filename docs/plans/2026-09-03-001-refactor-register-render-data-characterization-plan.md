...
- KTD9. **The adapter's three miswires are made impossible, not tested.** Extracting the projection moves the risk to the literal that names its inputs, and three miswires there are invisible to a pure unit test *and* to every existing spec — the peer demonstrated each. The plan removes the possibility rather than adding a test that would have to reach into the host to see it (Ch. 12: prefer the design that cannot express the error).
  ...
  3. **Same-typed primitive pairs in the input.** Two booleans named beside each other — TaskNotes presence and the date-indicator flag are the pair the peer demonstrated — swap silently: the projection cannot see the host literal, and the legend fixture supplies `true` for both, so a swap renders indicators in a view that disabled them and reports TaskNotes absent, with every gate green. Nesting the two under named groups does **not** fix this — the values can still be swapped inside the literal and typecheck. The input type therefore **brands** each same-typed primitive it carries at its reader, so two of them are not interchangeable and a swap is a compile error. As a second, independent gate, U1 runs `test/specs/gantt-date-handling.e2e.ts`, whose fixture is standalone with date indicators enabled — a false/true state, unlike the legend spec's true/true — so a swap of this pair changes what that spec asserts. This is the same remedy as the mapping pair above, applied as a rule over the whole input rather than to one instance — the plan states the rule and the characterization enumerates the pairs it covers.
