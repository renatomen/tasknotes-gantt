...
- **Host-side miswire (R5a):** the adapter now names about twenty values into one object. The three miswires that no tier could observe are made impossible by KTD9. What remains is a swap of two same-typed booleans, which `test/specs/gantt-legend.e2e.ts` observes for the legend-facing pair; the unit's verification runs it unchanged as the composition check, and the PR body names any pair even that spec cannot discriminate.
