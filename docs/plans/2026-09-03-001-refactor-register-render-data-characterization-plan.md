...
- R3. The render-contract projection lives in an owned module as a **pure function** over a single typed input value and returns the contract; `register.ts` keeps every host read in its current position, the cache write, the calendar-shading call and its registrations, and the call to the projection. The two option coercions inlined in the assembly move to the readers module beside their siblings.
...
U1 also touches **rank 4, `src/controller/GanttController.ts`** (same report, ranked list entry 4): it gains one builder that returns an already-bound refresh-generation closure, joining the two derivation builders that already live beside it (KTD9.1). The touch is argued rather than incidental — it is what makes a host miswire impossible instead of merely tested — and it adds no concern: the file's derivation-authority concern already owns `buildDeriveEstimate` and `buildDeriveSpan`, and this is its third member. Its Definition of Done obligation is the same as rank 2's: no metric regresses, and the closing report re-enumerates both entries.
...
- **Host-side miswire (R5a), with its two residuals named.** The adapter now names about twenty values into one object; the three miswires no tier could observe are made impossible by KTD9. Two gaps remain, and the plan states them rather than claiming cover:
  1. **Same-typed primitive swaps are closed two ways, and the PR body says which applies to each pair.** Branding at the reader makes a swap a compile error (KTD9.3); for pairs branding cannot reach, `test/specs/gantt-date-handling.e2e.ts` discriminates the TaskNotes-present / date-indicators pair because its fixture is standalone with indicators enabled, where the legend spec supplies true for both and stays green either way. Any pair neither gate covers is named in the PR body as a residual rather than papered over.
