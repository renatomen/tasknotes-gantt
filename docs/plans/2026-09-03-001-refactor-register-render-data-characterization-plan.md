...
- **The width ordering contract becomes data flow.** The projection returns the contract *and* the first-column width; the host writes its cache from the return value. There is no ordering to pin because there is no interleaving left. (The cache has a second consumer — the grid-width persist path during a user drag — so it stays a host field.)
...
- KTD1. **The render-contract projection is a pure function over one value.** `src/bases/ganttRenderContract.ts` (name directional) exports a function from a single typed input to `{ data: GanttData; firstColumnWidth: number }` — no `this`, no Obsidian, no Bases config, no clock, no globals. This is Ch. 11's essential-versus-accidental cut applied to the exact shape Ch. 12 asks for: the input is a **value**, not a port of getters. A getter port would have been the same host reads wearing an interface — nineteen members to observe five decisions, a leaky abstraction by Ch. 12's test — and it is what the superseded design proposed. The value is what makes the tests plain: construct an input, call, assert the contract.
...
- KTD3. **Two of the three ordering contracts dissolve; the third stays host-side.** (1) The name-column width was written mid-pass and read later through `getTableWidth()`; the projection now returns the width and the host writes its cache from the return, so the ordering becomes data flow. The cache keeps its second consumer, the grid-width persist path during a user drag. (2) `reconcileCalendarSelectionAlias()` keeps its position at the head of the pass and its config write; it stays in `register.ts` and is not part of the projection. (3) The raw-versus-effective mapping split stays split and becomes two named parameters of the input value, which is stronger than a comment: a caller cannot pass one where the other belongs without saying so. The recipe's hard-won rule still applies — pin them, never DRY-unify them.
...
- **Goal:** `src/bases/ganttRenderContract.ts` exports a pure projection from one typed input value to `{ data: GanttData; firstColumnWidth: number }`; `buildGanttData` keeps its alias reconcile, its fan-out, every host read in its current position, the calendar-shading call, and the width-cache write, and calls the projection with the values it holds.
...
- **Files:** `src/bases/ganttRenderContract.ts` (new), `src/bases/register.ts`, `test/unit/ganttRenderContract.test.ts` (new), `docs/backlogs/backlog.md` (park the `buildCalendarShading` separation-of-concerns finding).
- **Approach:**
  ...
  2. Move the literal and the derivations that feed it — the grid columns, the cell editors, the first-column width, the legend feeders and the legend literal — into the module. Everything that reads the host stays behind.
  3. Return the width alongside the contract; the host writes `lastFirstColumnWidth` from the return, keeping the cache its drag-time consumer needs.
  4. Write the module's tests first, against current behavior, then delete the register-side original.
- **Test scenarios (plain values in, contract out — no view, no harness, no cast):**
  ...
  - Function-valued members are pinned by call, not by type: the refresh-generation member returns the controller's next generation when invoked detached from the contract; the two derive closures answer what their inputs answer; the drag-mode supplier is the supplier that was passed in, and calling it after the projection returns yields the current value. Red: bind any of them as a bare method reference.
  - Width: the returned width is the first column's width for the given columns, and the returned contract's grid width is the persisted value when one is supplied and that width when none is. Red: return the fallback in both cases.
  ...
  - Locale: the contract's locale is the locale value supplied, and it is the same value the cell-data inputs were formatted with. Red: publish one and format with another.
  - Mutation checks: each "Red:" above is run, observed red, reverted, and the applied change printed as evidence.
- **Host-side miswire (R5a):** the adapter now names about twenty values into one object; two same-typed booleans swapped there typecheck. The existing legend and toolbar behavior in `test/specs/gantt-legend.e2e.ts` observes the composed result, and the unit's verification runs it unchanged as the composition check. Where a swap would be invisible even there, the PR body says so.
