...
For `scaleLabel`, instrument the measured AE4/AE5 action site and its preceding zoom/scroll deliveries. Capture source/delivery generations and one atomic, JSON-safe scalar snapshot when the test establishes `expectedState`, then at its existing assertion immediately after `openLegend()`, and on failure: DOM and SVAR-store scroll; numeric `xArea.from`, `to`, `start`, and `end`; `_scales` start/end, length/min unit and width; numeric `diff(end,start,lengthUnit)` evaluation; selected-state facts; and the logical scale-cell index/value plus rendered identity, label, and geometry. Verify this shape through the actual WebDriver/JSON retrieval path; never serialize the `diff` function.
