...
The spike (`test/probe/`, run with `npm run probe:svar`) built and proved the renderer. Apply these before executing this plan:

- **Width comes from an end date, not `duration * cellWidth`.** `cellWidth` is pixels per `lengthUnit` cell while `duration` is in the store's separate `durationUnit`; multiplying them is wrong whenever the units differ. Derive `end = start + duration` in the duration unit, then `width = round(diff(end, start, lengthUnit, inclusive) * cellWidth)`. U4's original test scenario asserted the wrong relationship.
- **Outer-bar transparency needs no registered cue type.** KTD8's cue-class-plus-registration machinery is unnecessary: a `:has(> .wx-segments)` rule targets exactly the segmented bar, and because a segment element never contains a segments container, it cannot blank the segments. This removes the `INSTANCE_CUE_SUFFIXES` cross-product expansion and its ordering contract entirely.
- **SVAR's whole-bar progress fill must be suppressed explicitly.** SVAR only skips its own progress wrapper when `splitTasks` is true, and the MIT build forces that false — so it paints a fill spanning the entire bar underneath the segments. Hide the bar's direct-child progress wrapper; the child combinator matters so per-segment fills survive.
...
- **Date math must be calendar-aware.** A millisecond-based day difference drifts by an hour across a DST boundary and shifts segment offsets by a pixel.
