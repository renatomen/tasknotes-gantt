...
- **Early-stop on a positive matched-parent signal only — never on emptiness** (R2). An empty/unchanged index is indistinguishable from a warmup lull, so any "content-stable across two attempts" or "any global edge" signal false-stops mid-warmup and leaves Show-all permanently under-expanded (this was a regression caught in doc review round 2). The only safe positive signal is "the index resolved a relationship edge touching a *currently-matched* task," computed at build time **and keyed on the edge type the active mode's expansion actually consumes** — Show-all keys on `childrenByPath` (it pulls descendants from there), Inherit keys on `parentsByPath` (it nests via those edges). *(Correction applied post-implementation per Codex review on PR #169: the original spec keyed on `childrenByPath` **OR** `parentsByPath` regardless of mode, but under partial warmup a matched task's parent edge can resolve before its children — so in Show-all the `OR` would false-report ready on the parent edge, cache the partial index, and leave the children absent. Mode-aware keying fixes this; `childrenByPath` is keyed by parent path — companionResolve.ts:55.)* The attempt cap is the **sole** backstop for the no-edges case.
...
- **Approach:**
  ...
  - Compute a readiness signal during `buildSnapshot` and expose it (e.g. `readinessStatus(): { companionActive: boolean; matchedEdgesResolved: boolean }`). `companionActive = this.companionAccessor !== null`. `matchedEdgesResolved = ` at least one matched (raw) task path appears as a key in the **mode-relevant** relationship map: `relationshipIndex.childrenByPath` in Show-all (the descendant-pull edge) or `relationshipIndex.parentsByPath` in Inherit (the nesting edge). Empty matched set ⇒ vacuously `true` (nothing to heal → window never starts); empty/not-ready index ⇒ `false`. *(As-built per Codex review on PR #169 — the original `childrenByPath` OR `parentsByPath` keying was too weak for Show-all under partial warmup; see Key Technical Decisions.)*
