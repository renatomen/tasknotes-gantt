...
- **DoD:**
  - `grep -cE '^## (Bar color mode|Bar color source)' website/docs/settings/appearance.md` prints 0 and `grep -c '^## Theme mode' website/docs/settings/appearance.md` prints **1** — the deletion guard and the do-not-delete guard. Read the printed **value**, not the exit status: `grep -c` exits 1 when it counts zero, so the passing state of the first one looks like a failure to `set -e`.
