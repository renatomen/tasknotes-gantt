...
# Fetch ONCE to a file and bail if git cannot answer. Piping `git show` into
# `grep -c` fails OPEN for the absence probe: a failed show leaves grep reading
# empty stdin, which prints 0 — the documented pass value — so the probe reports
# "U1 landed" precisely when git could not tell.
git show origin/main:website/docs/settings/appearance.md > /tmp/og-appearance.md || exit 1
# Assert on the VALUE, never the status: `grep -c` exits 1 when it counts zero,
# so the expected 0 below would abort the probe under `set -e`.
gone=$(grep -cE '^## (Bar color mode|Bar color source)' /tmp/og-appearance.md || true)
added=$(grep -cE '^## (Bar fill|Bar strip|Default legend position)' /tmp/og-appearance.md || true)
[ "$gone" -eq 0 ] && [ "$added" -eq 3 ] && echo LANDED || echo NOT-LANDED
```
...
- **In:** `website/docs/**`, `website/mkdocs.yml`, `docs/media/**`, new capture spec modules under `test/`, `docs/conventions/visual-assets.md` (R6 only), and `docs/releases/**` with its generated index `docs/releases.md`.
...
```bash
git diff --quiet origin/main...HEAD -- test/specs/gantt-calendar-editor.e2e.ts; rc=$?
case "$rc" in
  0) ;;                                   # unchanged — the only pass
  1) echo "ranked file changed"; exit 1;; # changed
  *) echo "git failed (rc=$rc)"; exit 1;; # could not tell — fail closed
esac
```

⚠️ **Read the status, never the output.** `test -z "$(git diff …)"` inspects captured stdout only: if the ref cannot be resolved, git writes to stderr, the substitution is empty and the guard passes — failing OPEN exactly when git is broken. `--quiet` puts the answer in the exit status, and the third branch above is what refuses to treat "could not tell" as "unchanged".
...
- **Files:** `website/docs/settings/appearance.md`, `website/docs/settings/timeline.md`, `website/docs/settings/fields.md`, `docs/conventions/visual-assets.md`, **plus every page carrying an inbound link to a renamed heading** — currently `website/docs/troubleshooting.md` and `website/docs/features/appearance.md` (its *Bar colors* section only; U2 owns its *Weekend shading* section). Renaming a heading changes its anchor, so leaving those out would ship dead links: `mkdocs --strict` cannot catch them until U2 adds the `validation:` block (R11). No ranked-defect file appears in this list.
...
- **Capture fixture — DEFERRED, and U1 ships without it.** ⚠️ The two panel captures were taken, reviewed and then **removed** (commit `0b30e359`), because the peer-review gate refuses any diff containing a binary file (`scripts/cross-model-peer-review.sh:285-287`, exit 14) and no image has reached `main` since that guard landed 2026-08-14. Without a receipt the pre-push hook blocks, so an image-bearing U1 is unpushable. The captures survive un-committed at `docs/media/_shots/` with the probe that produced them; re-adding them is a re-run, not a re-decision. **This blocks U2–U6 too — every one of them ships screenshots — so it is a campaign-level risk, filed in `docs/backlogs/backlog.md`, not a U1 footnote.** Re-capture recipe, for when the gate is fixed: the Appearance and Timeline view-option panels, staged from **`test/vaults/gantt-calendar`** (named per KD2 — "any committed fixture" is not a name). Single shots, `view-settings-groups-light.png` precedent. **Run the capture as `OBSIDIAN_TEST_VAULT="$(pwd)/.wdio-vault" npm run e2e:local -- --spec <spec>`** — an explicit throwaway path, and **never the cleared form**. ⚠️ `OBSIDIAN_TEST_VAULT=` does *not* enforce KD1: `scripts/e2e-local.mjs:16-17` is `process.env.OBSIDIAN_TEST_VAULT || LOCAL_VAULT`, so an empty value is falsy and gets replaced by a hardcoded fallback, and the Vite build **installs the plugin into whatever that resolves to** (`closeBundle` → `install-to-vault.mjs`). `.env` also defines the variable. A non-empty explicit path survives that `||`, is what `wdio.conf.mts:12-13` then copies from, and is gitignored (`.gitignore:20`).
- **DoD:**
  - This exits 0 — the deletion guard and the do-not-delete guard as one command: `[[ $(grep -cE '^## (Bar color mode|Bar color source)' website/docs/settings/appearance.md) == 0 && $(grep -c '^## Theme mode' website/docs/settings/appearance.md) == 1 ]]`
  ...
  - **Captures deferred** (see Capture fixture above): U1 lands without them, so this DoD line is knowingly unmet and the gap is recorded in `docs/backlogs/backlog.md`. When they return, both panels are opened and looked at before being referenced — a green gate is not a reviewed image.
...
- **DoD — the mutation set is the contract.**
  ...
  - **Direction that must be empty: every shipped control has exactly one heading, on its own group's page.** This is the direction that catches an undocumented, duplicated or misfiled setting, and it is the one U1b is accountable for.
...
- **Files:** `website/docs/features/calendars.md` (new), `website/docs/features/appearance.md` (**Weekend shading** section only — U1 already corrected its *Bar colors* section for the #312 rename), `website/mkdocs.yml` nav.
...
- **DoD:** every tab and top-level control in `src/editor/CalendarEditorForm.svelte` is either documented or listed in the PR body as deliberately out of scope with a reason — **"top-level control" means every element the form's own markup renders directly in a tab panel (button, input, select, toggle), not elements nested inside a child component**; enumerate them from the file and list the enumeration in the PR body so the denominator is reviewable. the base-pinned `git diff --quiet` check from § Ranked-defect review contract exits 0 (a bare `git diff --stat` is empty on any committed tree and proves nothing, and the `test -z "$(…)"` form fails open when git itself errors).
...
- **Work:** rewrite each feature entry as a short summary plus a link to its new page. Keep the Fixed section substantive — no docs pages back it, and bug fixes get no imagery (KD4). Keep the @donaldwdci credit, keep #311 linked with its open residual stated. **Leave the existing `<!-- release-date: YYYY-MM-DD -->` line in place, with a real date still in it** — it is a non-final placeholder that **U8 replaces**, because U7 runs first and cannot know the tag day. ⚠️ Do **not** blank it: `RELEASE_DATE_RE` (`releaseFiles.mjs:29`) matches only a literal `YYYY-MM-DD`, so an emptied comment throws at `:217-218` and takes `update-release-index --check` and the jest suite down with it. The validator throws only on an *absent or malformed* date, never a stale one, and `generate-release-notes-import.mjs:9` bakes whatever it finds into the in-app What's New bundle.
- **DoD:** `node scripts/update-release-index.mjs --check` exits 0 — **but note it is green before, during and after U7**: it reads only filenames (`update-release-index.mjs:57` is the whole per-release payload) and U7 renames nothing, so it cannot fail on anything U7 does. It is a supplement, not the gate. **The gate U7 must add and pass** is an executable link check with a non-zero exit: parse **every** Markdown link destination in the changed release file and classify each one — no prefix filter. A `https://tngantt.com/<path>` link resolves against `website/docs/<path>.md` or `<path>/index.md`; a `raw.githubusercontent.com` URL resolves against a file present in `docs/media/`; anything else — a relative link, a bare path such as `/features/calendars/`, another host, a typo'd domain — is **rejected**, because nothing else validates this file and an unrecognised shape must not pass by going unexamined (Material serves directory-style URLs, so accept both the `.md` and the `index.md` form). ⚠️ **It must also assert each image ref equals this note's own version.** The existing validator only requires a *release-shaped* ref (`releaseFiles.mjs:168` `isReleaseRef`) and then checks asset presence "against the current tree rather than its ref" (`:176-177`), so a beta.11 note pinning an image to `0.1.0-beta.10` passes every gate today and renders that older tag's bytes — an R6 violation no check sees. Parse each ref and require it to equal `0.1.0-beta.11`. `mkdocs --strict` does **not** see this file (R10). No manifest change in this PR.
...
- **DoD:** the tag exists, the GitHub release is published with the notes body, and the in-app What's New bundle regenerated from notes present on `main`. **Set the `<!-- release-date: YYYY-MM-DD -->` line to the tag day on `main`, in its own small PR, *before* cutting the release branch** — then cut `release/*` from that commit. `docs/releases/RELEASING.md:105-106` says the `release/*` branch is **never merged**, so a date correction committed there is stranded: `main` and every regenerated What's New bundle keep the provisional date, while leaving it uncommitted makes `npm version` refuse a dirty tree. Verify afterwards against `git log -1 --format=%ad --date=short <tag>`. **After the tag exists, confirm every `raw.githubusercontent.com/.../<tag>/...` URL in `docs/releases/0.1.0-beta.11.md` returns HTTP 200** — until the tag is cut those URLs 404 by design (`visual-assets.md`: release notes pin to the release tag), and this is the only point at which that can be checked. #311 is left open for the maintainer to close.
