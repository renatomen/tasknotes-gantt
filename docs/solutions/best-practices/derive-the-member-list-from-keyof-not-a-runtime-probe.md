...
switcher member added tomorrow would ship dead with every guard green.
...
- **A literal-key assertion.** `keyof T` widens to `string` the moment `T` gains a **string** index
  ...
  symbol index signature also widens `keyof`, but it *adds* `number` or `symbol` beside the literal
  keys rather than absorbing them, so `Record<keyof T, V>` still demands every named member. Measured
