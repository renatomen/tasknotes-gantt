...
Worse, the same plan already held the reasoning that made the fold free, three sections above the
guard it justified:
...
- [View display options live in presentation, not derivation](../architecture-patterns/view-display-options-in-presentation-not-derivation.md)
  — its scope-boundary paragraph defers type/token-bearing fields as "a separate, lower-severity
  concern — audit them". That audit has now landed (#469, #470) and found correctness bugs rather
  than a churn nuance; the paragraph needs updating.
