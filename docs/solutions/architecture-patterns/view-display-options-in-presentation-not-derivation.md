...
- **Search mechanism (engine 4) — UNMEASURED.** Procedure: reproduce a search type + clear on the real vault and read the logged `[OGDBG] onDataUpdated`-stack frames. *Bases-internal frames* ⇒ autonomous oscillation (search changes `R`, so the array can't be stabilized — verify the 500ms coalescer catches it; mitigate at the Bases-interaction layer only if not). *Our-plugin frames* ⇒ our re-read re-pokes (`reuseTasks` can't apply — entries genuinely change — so design a search-specific settle). Record the finding here, then implement the chosen mitigation. The config-toggle case already showed *zero plugin frames* (Bases autonomous); search's profile is genuinely different and not yet captured.
