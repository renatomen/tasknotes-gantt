...
- **Search mechanism (engine 4) — CLOSED. It was never a re-poke.** Clearing a Bases toolbar search genuinely changes the matched set, so `reuseTasks` correctly releases and a full re-read runs — that part is by design. The churn came from what happened *next*: the diff re-applied the entire companion-expanded set per-instance (~114k DOM mutations per swing, ~25s). Bases delivered a constant matched set throughout; the cost was ours. Bulk-reseed bounded it to 781 mutations, and the maintainer validated the fix in the real vault. **Do not go looking for a `getValue` re-poke here** — an earlier hypothesis said one existed, and it is wrong.
