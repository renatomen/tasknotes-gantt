...
symptoms:
  ...
  - "Dropping a field from the view's inline copy of a mapping leaves the entire suite green"
  - "`Record<keyof T, ...>` is not expressible because the shape being guarded is an anonymous inline object literal"
...
# "I cannot write a test that executes this" names a seam — and the extraction is sometimes the precondition for the guard, not its reward
...
The consequence is the part worth naming. **Every guard written against the filter had to
restate the mapping in order to observe it** — which makes the guard a second
implementation of the thing under test. The spec that existed said so in its own header, at
...
| 3 | It made a completeness guard **expressible at all** | Yes |

**A seam extraction can be the precondition for a guard rather than its beneficiary.**
`Record<keyof T, …>` requires a `T` the guard can *refer to*. Being an object literal is not by
itself the barrier — `keyof typeof value` reads a literal's keys, and this repository already does
it (`test/perf/generator/presets.ts:66`). The barrier is reachability: `typeof` still needs a
binding the test file can import, and this literal was an argument expression inside a module the
test tier replaces with a stub. There was nothing to name it from. The compile-time completeness table in the sibling learning
(`test/unit/rowVisibilityLiveSync.test.ts:287`) is keyed on `RowVisibilitySource` — an
interface that **did not exist** before #473. So the sequencing is not "extract, then get a
nicer guard": without the extraction that guard has no subject and cannot be written down.
...
**The guard in the sibling learning was gated on this extraction.** Before #473,
`src/bases/rowVisibility.ts` declared `RowVisibilityInput` and `RowVisibilityFlags` and
nothing else; `RowVisibilitySource` arrived in the same diff as `toRowVisibilityInput`
(`git show b3f6b92 -- src/bases/rowVisibility.ts`). Both halves of the settled mechanism —
`Record<keyof RowVisibilitySource, FieldDelivery>` at
`test/unit/rowVisibilityLiveSync.test.ts:287` and its degeneracy guard
`LiteralKeys<RowVisibilitySource>` at `:307-308` — name that interface. Neither is
expressible against an inline literal. The three defeated runtime probes documented in that
learning were all attempts to reach, by observation, a member list that could only be
*declared* once the projection had a name.
...
`test/unit/rowVisibilityLiveSync.test.ts` that reach it through the store (three visible-row-set
cases via `visibleIds` at `:162`, four staleness-scenario cases via `projections` at `:266`). The
...
- **When a rule needs a member list and its subject is anonymous.** An inline literal, an
  ad-hoc parameter shape, a destructured argument. Name the type first — the guard is
  usually trivial once there is something to put in `keyof`, and impossible until then.
...
**The guard the seam made expressible** (`test/unit/rowVisibilityLiveSync.test.ts:287-308`)
...
- [A probe sees the fields one call happens to touch — derive a member list from `keyof`, not from
  ...
  must look like; read this one for why it could not have been written the week before.
