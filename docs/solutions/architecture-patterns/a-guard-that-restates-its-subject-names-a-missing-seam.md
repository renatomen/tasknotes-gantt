...
applies_when:
  ...
  - "Logic lives in a module the fast test tier cannot execute â€" a `.svelte` file under a jest stub mapping, a template, a config block"
...
`Record<keyof T, …>` requires `T` to have a name. An inline object literal has no type to
take `keyof` of. The compile-time completeness table in the sibling learning
...
discriminator: the moved code is now executed by **21 passing jest cases** across two specs
(9 in `test/unit/rowVisibility.test.ts`, 12 in `test/unit/rowVisibilityLiveSync.test.ts`;
run at HEAD), including two cases named directly for the projection (`:84`, `:99`) and the
whole live-sync half, which reads the store through it (`:162`, `:266`). Before the
