...
`src/bases/rowVisibility.ts` declared two types — `RowVisibilityInput` and
`RowVisibilityFlags` — and no third; `RowVisibilitySource` arrived in the same diff as `toRowVisibilityInput`
