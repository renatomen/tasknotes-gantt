...
2. If it reads all-passed, grep the rest of the step log case-insensitively for `FAILED in|failing|exited with|error` (excluding known-benign 0-failing lines and WARN noise). If nothing turns up, **no spec failed** — do not keep searching for one.
3. Classify the run as infrastructure-class ("launcher-exit"), not spec-level flake. Check what runs after the suite *inside the WDIO process*: `onComplete` hook execution and reporter file writes — the nonzero exit happened somewhere in that in-process tail, invisibly. (Artifact upload is a separate workflow step in `.github/workflows/e2e.yml` and cannot fail the WDIO step.)
