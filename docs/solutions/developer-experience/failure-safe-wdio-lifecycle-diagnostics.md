...
Post-failure resampling is later evidence, not the failing poll. When the verdict needs same-checkpoint facts, keep the original lightweight readiness query as the sole gate. Only after that query observes the symptom should the harness capture the bounded root/target census, with a fixed interval measured from the preceding capture's completion. Eligibility comes from the capture's own missing-bar result, never the earlier trigger; a failed capture disables further detailed capture for that wait window, and any later completed poll without its own census invalidates saved readiness evidence. The CDP fallback may then select that saved boundary only when its checkpoint matches the primary failure; every other resample remains ineligible for a causal verdict ([`calendarItemsSourcesLifecycle.ts`](../../../test/specs/helpers/calendarItemsSourcesLifecycle.ts), [`calendarItemsSourcesDiagnosis.ts`](../../../test/specs/helpers/calendarItemsSourcesDiagnosis.ts)).
