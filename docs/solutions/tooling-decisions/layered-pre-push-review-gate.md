...
- `REQUIRED_LAYERS` names both layers, and **both** are required for **every** pushed ref tip. The one namespace the hook does not gate is the archival review-subject namespace `refs/e11-subjects/*` (`ARCHIVAL_SUBJECT_REF_PREFIX`), which preserves the E11 reviewer-benchmark corpus's subject commits so any clone can rebuild the ranges it records; those refs carry review *subjects*, not reviewed code, and they are validated rather than receipted — each is a write-once pin named by the full object id of the commit it carries (the first 31, created before that rule, carry 7-character abbreviations), and replacing or deleting one is refused.
...
- [Stage untrusted-adjacent review content as gitignored DATA files](stage-peer-review-content-as-data-files.md) — how this gate's untrusted inputs (the reviewed diff, the maintainability trend block) reach the layer-2 reviewer: named gitignored data files the prompt disclaims of instruction force, with the sentinel receipt and main-side measurement detailed there.
