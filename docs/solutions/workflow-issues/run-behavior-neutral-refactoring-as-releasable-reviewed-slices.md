...
Fix findings, create a new commit, and rerun both reviews because the reviewed commit changed. Before dispatching either reviewer, record `reviewed_sha=$(git rev-parse HEAD)` and give that same SHA to both. Immediately before recording, confirm `git rev-parse HEAD` still equals `reviewed_sha`; `record <layer>` stamps the current `HEAD` and cannot infer which commit the reviewer saw. Any intervening commit, amend, or rebase requires both reviews again. Only after both layers are clean should their [review receipts](../../../CONCEPTS.md#review-receipt) be recorded. The pre-push hook invokes `scripts/check-review-receipts.mjs`, whose required layers are `ce-code-review` and `cross-model-peer`.
