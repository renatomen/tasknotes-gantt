...
>
> **Resolution update (2026-06-27):** the presentation-layer engine is now **fixed** — all row-visibility options (Hide-top, Show-undated, Show-partial) moved to one composed `filter-tasks` predicate over a stable, visibility-free derivation (`shouldHideRow` + `GanttContainer.applyDisplayFilters`; controller `DatePolicyConfig` carries only `defaultDuration`). The data-read engine fix below (`reuseTasks` gate) remains in place — it is load-bearing (proven by the negative-control repro) and was **not** removed.
>
> **Resolution update (2026-06-28):** the **search→clear** engine (#161 U6) is now root-caused and fixed. Search is `R`-changing, so it legitimately disarms the `reuseTasks` gate (the matched entry set really did change) — which re-exposes a *third* engine: the per-instance id-keyed diff applying thousands of ops over the companion-expanded set (~25s bounded storm, not infinite). Fixed in PR #172 by bounding that cost — a `shouldBulkReseed` escape hatch routes wholesale-resultset diffs through one virtualized reseed. So this isn't a `getValue` re-poke (the gate is correct to release on a real `R` change); it is the diff-cost engine. See [svar-gantt-diff-sync-interactions.md](svar-gantt-diff-sync-interactions.md) → "bulk-reseed escape hatch". Reproduced deterministically via [../developer-experience/vault-as-code-faithful-repro.md](../developer-experience/vault-as-code-faithful-repro.md).
