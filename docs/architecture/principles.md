...
No Obsidian or TaskNotes property name is ever hardcoded. Field roles (start, due, status, priority, progress, estimate, parent, name) resolve from the user-configured field mappings; defaults are computed at one seam — when a backing system is present, an unset field resolves from that system's own configuration; date fields bottom out at its documented defaults (names living only in the seam), while a role the backing system itself leaves unmapped (status, priority) stays unset and read-only per round-trip symmetry; standalone, with no backing system, an unset role stays unresolved and read-only — and every consumer reads the resolved value. The raw view config keeps one narrow job: it answers "what did the user choose?", never "which property IS this field?". The only tolerated literals are the Obsidian built-ins `file.name` / `file.basename` as the name-column fallback.
