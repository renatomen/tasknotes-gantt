...
The calendar-note frontmatter uses self-explanatory keys with canonical RFC values. This table is the lossless-boundary proof required by [standards-alignment.md](standards-alignment.md): every schema field maps to an RFC 5545 (iCalendar) or RFC 7953 (VAVAILABILITY) construct — or, where this table records it, a sibling iCalendar-family construct (RFC 7986 `COLOR` and calendar-level `DESCRIPTION`), and `src/controller/calendar/rfcMapping.ts` is the executable projection, round-trip-tested in `test/unit/calendarRfcRoundTrip.test.ts`.
...
| `description` | `DESCRIPTION` (RFC 7986 §5.2) | Calendar-level description — RFC 5545 defines `DESCRIPTION` only on components; RFC 7986 adds it at calendar level. |
| `color` | `COLOR` (RFC 7986 §5.9) | CSS colour value; validated at consumption (`isSafeColor`). **Tracked deviation:** the schema accepts any non-empty string and the mapping copies it verbatim, so any accepted value that is not a CSS3 colour name (hex, functional forms, anything else) violates §5.9 — see the backlog entry; this row is not yet a lossless proof. |
